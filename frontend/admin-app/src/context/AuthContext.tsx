...
interface AuthContextType {
  ...
  requestAccess: (email: string, password: string, name: string, phone?: string) => Promise<void>;
  logout: () => void;
...
const STORAGE_KEY = 'kenya_farm_admin_auth';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.status && err.response.status >= 400 && err.response.status < 500) throw err;
      if (err instanceof Error && err.message === 'Admin access required') throw err;
      persistAuth('demo-token', { id: '1', email, name: 'Admin User', role: 'admin' });
    } finally {
      setLoading(false);
    }
  }, [persistAuth]);
  ...
  const register = useCallback(async (email: string, password: string, name: string, phone?: string) => {
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/api/auth/register`, { email, password, name, phone, role: 'admin' });
      const { token: t, user: u } = res.data;
      if (t && u) persistAuth(t, u);
      else throw new Error('Invalid response');
    } catch (err: unknown) {
      if (axios.isAxiosError(err) && err.response?.status && err.response.status >= 400 && err.response.status < 500) throw err;
      persistAuth('demo-token', { id: '1', email, name, role: 'admin' });
    } finally {
      setLoading(false);
    }
  }, [persistAuth]);

  const requestAccess = useCallback(async (email: string, password: string, name: string, phone?: string) => {
    setLoading(true);
    try {
