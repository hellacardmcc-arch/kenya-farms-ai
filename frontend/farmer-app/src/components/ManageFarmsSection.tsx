...
import './ManageFarmsSection.css';

const ManageFarmsSection: React.FC = () => {
  const { token } = useAuth();
  ...
  const handleAddFarm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = getApiErrorMessage(err, language === 'sw' ? 'Imeshindwa kuhifadhi shamba' : 'Failed to save farm');
      alert(msg);
    } finally {
      setAdding(false);
    }
  ...
  const t = {
    en: {
      title: 'Manage Farms',
      subtitle: 'Add, edit, view, and remove farms',
      addFarm: 'Add Farm',
      manageAll: 'Manage All Farms',
      edit: 'Edit',
      view: 'View',
      remove: 'Remove',
      farmName: 'Farm name',
    ...
    sw: {
      title: 'Dhibiti Mashamba',
      subtitle: 'Ongeza, hariri, angalia, na futa mashamba',
      addFarm: 'Ongeza Shamba',
      manageAll: 'Dhibiti Mashamba Yote',
      edit: 'Hariri',
      view: 'Angalia',
      remove: 'Futa',
  ...
  return (
    <div className="manage-farms-section">
      <div className="manage-farms-header">
        <div>
          <h3>🌾 {currentLang.title}</h3>
          <p className="manage-farms-subtitle">{currentLang.subtitle}</p>
        </div>
        <button type="button" className="manage-all-farms-btn" onClick={() => navigate('/farms')}>
          {currentLang.manageAll} →
        </button>
      </div>

      <div className="manage-farms-actions">
        <button
          type="button"
      ...
      {farms.length === 0 ? (
        <p className="manage-farms-empty">{currentLang.noFarms}</p>
      ) : (
        <div className="manage-farms-list">
          {farms.map((farm) => {
            const farmCrops = getCropsForFarm(farm.id);
            const isViewing = viewingFarm?.id === farm.id;
            return (
              <div key={farm.id} className={`manage-farm-card ${isViewing ? 'expanded' : ''}`}>
                <div className="manage-farm-card-header" onClick={() => setViewingFarm(isViewing ? null : farm)}>
                  <div className="manage-farm-card-title">
                    <span className="manage-farm-name">🌾 {farm.name || farm.unique_code || farm.id}</span>
                    <span className="manage-farm-area">
                      <AreaDisplay hectares={farm.area_hectares ?? 0} />
                    </span>
                  </div>
                  <span className="manage-farm-crops-badge">
                    {farmCrops.length} {currentLang.cropsCount}
                  </span>
                  <span className="manage-farm-expand">{isViewing ? '▲' : '▼'}</span>
                </div>
                <div className="manage-farm-actions">
                  <button type="button" className="manage-btn-edit" onClick={() => openEdit(farm)}>
                    ✏️ {currentLang.edit}
                  </button>
                  <button
                    type="button"
                ...
                {isViewing && (
                  <div className="manage-farm-detail">
                    <div className="manage-farm-detail-row">
                    ...
                    <div className="manage-farm-detail-row">
                      ...
                      <span><AreaDisplay hectares={farm.area_hectares ?? 0} /></span>
                    </div>
                    {farm.unique_code && (
                      <div className="manage-farm-detail-row">
                    ...
                    {(farm.latitude != null || farm.longitude != null) && (
                      <div className="manage-farm-detail-row">
                        ...
                        <span>{farm.latitude}, {farm.longitude}</span>
                      </div>
                    )}
                    {farmCrops.length > 0 && (
                      <div className="manage-farm-crops-list">
                        <span className="label">{currentLang.cropsCount}:</span>
                        <ul>
                          {farmCrops.map((c) => (
                            <li key={c.id}>
                              {language === 'sw' ? (c.swahili_name || c.name) : c.name} · <AreaDisplay hectares={c.area_hectares ?? 0} />
                            </li>
                          ))}
                        </ul>
                      </div>
                    )}
                    <button type="button" className="manage-btn-crops" onClick={() => navigate('/crops')}>
                      {currentLang.viewCrops} →
                    </button>
                  </div>
                )}
              </div>
      ...
      {showAddForm && (
        <div className="manage-farm-overlay" onClick={() => setShowAddForm(false)}>
          <div className="manage-farm-modal" onClick={(e) => e.stopPropagation()}>
            <h4>{currentLang.addFarm}</h4>
            <form className="manage-farm-form" onSubmit={handleAddFarm}>
              <div className="form-row">
                <label htmlFor="mf-farm-name">{currentLang.farmName}</label>
                <input
                  id="mf-farm-name"
                  type="text"
                  value={farmForm.name}
              ...
              <div className="form-row">
                <label htmlFor="mf-farm-location">{currentLang.farmLocation}</label>
                <input
                  id="mf-farm-location"
                  type="text"
              ...
              <div className="form-actions">
                ...
                <button type="submit" disabled={adding}>{adding ? '...' : currentLang.addFarm}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {editingFarm && (
        <div className="manage-farm-overlay" onClick={() => setEditingFarm(null)}>
          <div className="manage-farm-modal" onClick={(e) => e.stopPropagation()}>
            <h4>{currentLang.edit} — {editingFarm.name || editingFarm.id}</h4>
            <form className="manage-farm-form" onSubmit={handleUpdateFarm}>
              <div className="form-row">
                <label htmlFor="mf-edit-farm-name">{currentLang.farmName}</label>
                <input
                  id="mf-edit-farm-name"
                  type="text"
                  value={farmForm.name}
                  onChange={(e) => setFarmForm({ ...farmForm, name: e.target.value })}
                  required
              ...
              <div className="form-row">
                <label htmlFor="mf-edit-farm-location">{currentLang.farmLocation}</label>
                <input
                  id="mf-edit-farm-location"
                  type="text"
                  value={farmForm.location}
                  onChange={(e) => setFarmForm({ ...farmForm, location: e.target.value })}
                  placeholder={currentLang.farmLocation}
              ...
              <div className="form-actions">
                <button type="button" onClick={() => setEditingFarm(null)}>{currentLang.cancel}</button>
                <button type="submit" disabled={saving}>{saving ? '...' : currentLang.save}</button>
              </div>
            </form>
          </div>
        </div>
...
export default ManageFarmsSection;
